      ...
      for (let j = 0; j < projectFiles.length; j++) {
        if (j !== i) {
          if (projectFiles[i].fsPath.startsWith(projectFiles[j].fsPath)) {
            window.showWarningMessage(
              `dbt Power User detected a project located in ${projectFiles[i]} that is contained inside another project ${projectFiles[j]}. This is an unsupported configuration. If you believe this is a valid configuration, please open a github issue.`
            );
          }
        }
      }
