...
export const executePythonCode = async (code: string, python: PythonBridge) => {
  // Capture the output using a custom wrapper
  // Capture the output using a custom wrapper
  const wrappedCode = `
    import io
    import sys
  ...
  const a = 1,
    b = 3;
  // return await this.python`exec("""${wrappedCode}""")`;
  //             this.python.ex`import math`;
  // return this.python`hello(${code.replace(/"/g, '\\"').replace(/\n/g, '')})`;
  ...
  // const realKernel = new KernelConnection({
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      try {
        // output = await python.ex`exec("""${code}""")`;
        await python.ex`import sys`;
        await python.ex`from io import StringIO`;
        await python.ex`sys.stdout = StringIO()`;
        await python.ex`sys.stderr = StringIO()`;
        await python.ex`${code}`;
      } catch (error) {
        const stderr = await python`sys.stderr.getvalue()`;
        reject(stderr);
      }
      // Capture stdout and stderr
      const stdout = await python`sys.stdout.getvalue()`;
      const stderr = await python`sys.stderr.getvalue()`;
      // Apply the output to the cell
