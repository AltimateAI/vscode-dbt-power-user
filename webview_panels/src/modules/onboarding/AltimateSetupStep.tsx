...
const AltimateSetupStep = ({
  ...
  const handleSaveKey = async () => {
    ...
    try {
      ...
      // Wait a bit to show success message, then advance to integration step
      setTimeout(() => {
        setSuccess(false);
        onComplete?.();
      }, 1500);
  ...
  // Phase: "key" — if already configured, auto-advance to integration step
  if (phase === "key" && isAltimateConfigured) {
    return (
      <div className={classes.altimateKeyContainer}>
        {renderProgressCard()}
        <Alert
  ...
  // Phase: "key" — show API key setup form
  return (
    <div className={classes.altimateKeyContainer}>
      {renderProgressCard()}
