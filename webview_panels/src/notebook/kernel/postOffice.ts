...
class VsCodeMessageApi implements IMessageApi {
    ...
    public register(msgCallback: (msg: WebviewMessage) => Promise<void>) {
        ...
        if (!this.vscodeApi) {
            panelLogger.error('The vscode api is not set');
        }
        if (!this.registered) {
    ...
    public sendMessage(type: string, payload?: any) {
        ...
        } else if (type === 'IPyWidgets_panelLogger.info') {
            panelLogger.info(`Logging message ${type}, ${payload}`);
        } else {
            panelLogger.info(`No vscode API to post message ${type}`);
        }
    }
...
export class PostOffice implements IDisposable {
    ...
    public sendUnsafeMessage(type: string, payload?: any) {
        if (this.messageApi) {
            ...
            panelLogger.log('Message not sent', type, payload);
        } else {
            panelLogger.info(`No message API to post message ${type}`);
        }
    }
